...
#
# $Id: build-test-env-for-find.sh 830 2013-03-07 23:15:50Z tom $
#
# Macht eine Directoryhierarchie für `find`
#
...
# und ein tiefes langes directory
    (
        set -e
        local pathname="$(uuidgen)"
        while [ "${#pathname}" -lt "4095" ]; do
            mkdir "$pathname"
            pathname="$pathname/$(uuidgen)"
        done
    ) || : # ignore errors
}
...
# fixup the permissions
  chmod "$perms" "char-device-$perms" "block-device-$perms" "directory-$perms" "fifo-$perms" "plain-file-$perms" "socket-$perms"
done
...
for perms in u={r,-}{w,-}{x,-}{s,},g={r,-}{w,-}{x,-}{s,},o={r,-}{w,-}{x,-}{t,}; do
    : > "file-${perms}"	# save fork(2)+exec(2) avoiding "touch"
    mkdir "dir-${perms}"
    chmod "${perms//-}" "file-${perms}" "dir-${perms}"
done
